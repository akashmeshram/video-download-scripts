...
for dep in yt-dlp ffmpeg zenity; do
...
if [ ${#MISSING_DEPS[@]} -gt 0 ]; then
  ...
  if [[ " ${MISSING_DEPS[*]} " =~ " ffmpeg " ]] || [[ " ${MISSING_DEPS[*]} " =~ " zenity " ]]; then
    echo -e "For ffmpeg and zenity: ${GREEN}sudo apt install ffmpeg zenity${NC}"
  fi
...
echo -e "Example usage: ${GREEN}$SCRIPT_NAME${NC}"
